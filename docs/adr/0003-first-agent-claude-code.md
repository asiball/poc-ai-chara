...
**Claude Code を最初の Agent とする。** Codex は MVP 後の最初の拡張として `agents.json` に追加し、動作検証する。
...
3. **開発者自身が Claude Code を日常的に使っている**(本リポジトリの作業も Claude Code で行っている)ため、動作の妥当性を判断しやすい
4. Codex を先にしない理由は「劣る」からではなく、アダプタが 2026-07 に全面書き換え(Rust → TS)されたばかりで、挙動の安定を見極めたいため

## 前提と注意(重要)

- **サブスクリプションでの利用可否は Anthropic の利用条件に依存し、変更され得る**。本プロジェクトは次を守る
  ...
  - 開発・検証は **API キー(Console)でも必ず通す**。配布時の既定案内は API キー経路にする
  - 個人が自分のサブスクで自作クライアントを使うことが許容されるかは **公式に確認できていない**。README にその旨を明記する
- **製品名に「Claude Code」を含めない**。Agent 名の表示は「Claude Agent」「Powered by Claude」の範囲に留める
- アダプタのバージョンを固定する(`npx @agentclientprotocol/claude-agent-acp@0.78.0`)

## 結果

- `agents.json` の既定エントリは `claude` のみ
- 受け入れ条件(`docs/mvp.md` §3)は Claude Code で検証する
- Codex 対応は MVP 後に別 ADR を書かずに `agents.json` 追加 + 検証で進める(アーキテクチャ上の変更が不要なため)
